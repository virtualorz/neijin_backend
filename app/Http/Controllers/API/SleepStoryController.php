...
use App\Core\Controllers\SleepStory\SleepStoryContract;
use App\Models\SleepStory;
...
class SleepStoryController extends Controller implements SleepStoryContract
{
    ...
    public function create(ServerRequest $request): Model
    {
        ...
        $payload = $this->file_column_process_service->create(payload:$payload,model:SleepStory::class,upload_dir:'SleepStory');
    ...
    public function update(ServerRequest $request): Model
    {
        return DB::transaction(function() use($request){
            ...
            $db_data = $this->repository('SleepStory')->read_models(new ReadListParamsDto(filter:$filter,per_page:0));
            if(count($db_data) === 0){
                throw new ControllerException('找不到此筆資料');
            }
            $db_data = $db_data->toArray()[0];
            $payload = $request->validated();
            $payload = $this->file_column_process_service->update(payload:$payload,model:SleepStory::class,db_data:$db_data,upload_dir:'SleepStory');
