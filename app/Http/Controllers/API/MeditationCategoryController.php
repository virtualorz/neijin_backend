...
use App\Core\Controllers\MeditationCategory\MeditationCategoryContract;
...
class MeditationCategoryController extends Controller implements MeditationCategoryContract
{
    ...
    public function create(ServerRequest $request): Model
    {
        ...
        $payload = $this->file_column_process_service->create(payload:$payload,model:MeditationCategory::class,upload_dir:'MeditationCategory');
    ...
    public function update(ServerRequest $request): Model
    {
        return DB::transaction(function() use($request){
            ...
            $db_data = $this->repository('MeditationCategory')->read_models(new ReadListParamsDto(filter:$filter,per_page:0));
            if(count($db_data) === 0){
                throw new ControllerException('找不到此筆資料');
            }
            $db_data = $db_data->toArray()[0];
            $payload = $request->validated();
            $payload = $this->file_column_process_service->update(payload:$payload,model:MeditationCategory::class,db_data:$db_data,upload_dir:'MeditationCategory');
